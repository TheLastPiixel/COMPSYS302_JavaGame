...
public abstract class Creature extends Entity{
    ...
    public void moveX() {
        if (xMove > 0) {        //moving right
            ...
            if(!collisionWithTile(tx,(int)(posY + bounds.y) / Tile.TILEHEIGHT) && !collisionWithTile(tx, (int) (posY + bounds.y + bounds.height)/Tile.TILEHEIGHT)) { // check if top right is colliding
                posX += xMove;
    ...
    public void moveY(){
        if(yMove < 0){ //up
            ...
            if(!collisionWithTile((int)(posX + bounds.x) / Tile.TILEWIDTH, ty) && // checks top left and top right for collisions
            ...
            int ty = (int)(posY + yMove + bounds.y + bounds.height) / Tile.TILEHEIGHT; // set to bottom of collision rectangle

            if(!collisionWithTile((int)(posX + bounds.x) / Tile.TILEWIDTH, ty) && //checks bottom left and right for collisions
                    !collisionWithTile((int)(posX + bounds.x + bounds.width) / Tile.TILEWIDTH, ty)){
                posY += yMove;
            }else{
