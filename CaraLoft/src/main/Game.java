...
public class Game implements Runnable { // going to be the main game class, start and run and close everything
	...
	public String title;
	private Thread thread; // a thread is like a mini separate program, will run class seperate to the
							// other code
	...
	private void init() {
		...
		gameState = new GameState(handler); // can initialize it since gamestate extends state, so we initialise as a
											// gamestate but declare it as a state above
	...
	private void render() { // renders everything in the game
		bs = display.getCanvas().getBufferStrategy(); // this will set our current buffer strategy to the bs of the
														// canvas
	...
	public void run() { // need this if using runnable and threads
		...
		while (running) {
			...
			delta += (now - lastTime) / timePerTick; // will get the amount of time passed (now - time passed) and
														// divide by the max amount of time allowed
